...
<!-- 										<button type="button" class="emphasis__btn ml-2" onclick="overlapCheck('L')" style="display: inline-block;">중복확인</button> -->
										<button type="button" class="emphasis__btn ml-2" onclick="duplicateCheck()" style="display: inline-block;">Duplicate check</button>
...
<!-- 								<th  colspan="2">*자릿수</th> -->
								...
								<td  colspan="3">
									<input type="text"
										...
										id="lrgc_acph" 
										value=""
										onkeydown="javascript:fnOnlyNumber();" class="txt wx150" min="0" max="20" maxlength="2" />
								</td>
...
<!-- 							<th colspan="2" style="vertical-align: middle;">숫자값5</th> -->
    ...
    <script type="text/javascript">
	    ...
	    function goSave(){
	    	...
	    	if(checkId != lrgc_cd){						// 대분류 코드 중복 확인 후 아이디 변경 여부 
	    		alert(i18n.message_079); //"아이디가 변경되었습니다.\n중복확인을 다시해주세요."
	    		return;
	    	}
			var formData = $("form[name='lrgc_insertForm']").serializeObject();

			console.log("formData json >>>" + JSON.stringify(formData));
			$.ajax({
	     		type : "post",
	     		url : "/risCodeInsertData.do?",
